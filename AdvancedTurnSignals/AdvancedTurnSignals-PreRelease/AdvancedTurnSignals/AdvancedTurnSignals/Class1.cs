...
namespace AdvancedTurnSignals
{
    public class Class1 : Script //Scriptは継承。継承すると継承元の情報を利用できる。(この場合はSHV.NETの情報。なお、スクリプト作成には必須。）
    {
        ...
        private static bool use_button = false; //ボタン設定を使用するか
        private static string raw_leftC; //ボタン設定を用意
        private static string raw_rightC; //ボタン設定を用意
        private static string raw_hazardC; //ボタン設定を用意
        private static Control leftC; //ボタン設定を用意
        private static Control rightC; //ボタン設定を用意
        private static Control hazardC; //ボタン設定を用意
        ...
        private static bool autoon = false; //自動点灯
        private static string raw_on_angle = "10"; //自動点灯する角度
        private static Single on_angle = 10; //自動点灯に最低限必要な角度(コード内で使用)
        private static string raw_on_speed = "10"; //自動点灯する速度
        private static Single on_speed = 10; //自動点灯できる最高速度(コード内で使用)
        private static bool use_sound = true;
        private static bool inter_sound = true; //カスタムサウンドロード失敗時にデフォルトサウンドを使用するかのフラグ
        #endregion
        ...
        */
        ...
        { 
            ...
            if (enabled && cv != null && available(cv)) //有効かつ方向指示器が動作できる車に乗っている
            {
