...
非推奨ですが、方向指示器の代わりにヘッドライトを使用した制御も可能です。_（DiNaganoさんからのリクエスト）_
...
> [!CAUTION]
> Advanced Turn Signalsは[GitHubのReleasesページ](https://github.com/QuestDragon/GTAV_AdvancedTurnSignals/releases)か、[5mods](https://www.gta5-mods.com/scripts/advanced-turn-signals)でしか配布しておりません。
> 
> そのほかのサイトではウイルス混入などの危険性がございますので、これ以外のサイトではModをダウンロードしないでください。
> 
> また、安定版をご希望の場合は、5modsでのダウンロードがオススメです。GitHubではAdvanced Turn Signalsの全てのバージョンがダウンロードできますが、一部不安定なバージョンが公開されておりますので、ご利用の際はご注意ください。

| [Advanced Turn Signals](https://github.com/QuestDragon/GTAV_AdvancedTurnSignals/releases/latest/download/AdvancedTurnSignals.zip) | [ScriptHookV](http://dev-c.com/gtav/scripthookv/) | [レガシー版のみ：ScriptHookV DotNet 3.7.0 Nightly](https://github.com/scripthookvdotnet/scripthookvdotnet-nightly/releases/latest) | [エンハンス版のみ：ScriptHookV DotNet Enhanced](https://www.gta5-mods.com/tools/script-hook-v-net-enhanced) |
| ------------- | ------------- | ------------- | ------------- | 
...
**HeadlightMode**：方向指示器を点滅させる代わりにヘッドライトを点滅させます。方向指示器が装備されていない車両でもAdvanced Turn Signalsを使用したい方に向けた機能ですが、動作が不安定になる可能性があるため、非推奨としています。
...
> [!NOTE]
> KeyboardCompはキーボードユーザーのための機能です。アナログコントローラー等をお使いの方で自動消灯機能をご利用になる場合は、この機能をオフにしてください。
> KeyboardCompが有効の場合、アナログコントローラーでの自動消灯機能が動作しなくなります。

**AutooffDuration**：`KeyboardComp`が有効になっている状態で`OffAngle`の値まで戻った際に方向指示器を自動消灯する所要時間をミリ秒単位で指定できます。※約200ミリ秒以下にすると正常に動作しない可能性があります。即時自動消灯を行う場合はKeyboardCompを無効にしてください。
...
**UseLeverSound**：方向指示器が自動点灯した際にレバー音を鳴らすかどうかの設定です。`UseSound`がtrueである必要があります。「*true*」で有効、「*false*」で無効になります。ただし、設定条件が正しくない場合、スクリプトModロード時に一時的に有効になります。
...
**HeadlightMode**を*True*にしている場合は方向指示器の代わりにヘッドライトが点滅します。
...
部分的にデフォルト設定を使用する場合はタグ名だけ書いておいてください。タグを丸ごと省略することはしないでください。（例：\<HAZARD_BUTTON /> または \<HAZARD_BUTTON>\</HAZARD_BUTTON>）

> [!TIP]
> 無音にしたい場合は無音の音声ファイルをご用意いただき、ファイル名を指定すると無音に出来ます。
> ※ファイル名を指定しない場合、デフォルトサウンドが使用されます。
>
> なお、`InterSound`を*False*にすることでも無音に出来ますが、該当の車両に乗り換える度に警告が表示されます。
> 煩わしい場合は無音ファイルを指定することをオススメします。
